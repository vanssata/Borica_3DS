 ...
 * или еквивалентите им за рeанла среда, изпълните скрипта и не сработи, значи  със сигурност имате проблем в ключовете.
 *
 */
class Boorica
{
    ...
    /** Терминал ИД издаден от банката */
    private $terminal_id;

    /** Частния ключ с който е генериран ключа изпращан към Банката за тестовия терминал */
    private $merchant_test_private_key;
    /** Паролата за ключа  */
    private $merchant_test_passphrase=null;

    /** Частния ключ с който е генериран ключа изпращан към Банката за реалния терминал */
    private $merchant_real_private_key;
    /** Паролата за ключа  */
    private $merchant_real_passphrase = null;

    /** Публичния ключ издаден от банката за тестовата среда */
    private $borica_test_public_key;
    /** Публичния ключ издаден от банката за реална среда */
    private $borica_real_public_key;
    /** Линк за изпращане на формата към тестова среда */
    private $test_action = 'https://3dsgate-dev.borica.bg/cgi-bin/cgi_link';
    /**
     * Линк за изпращане на формата към реална среда
     * Тук очаквам поясние от борика дали не трябва да е https://3dsgate.borica.bg/cgi-bin/cgi_link
     */
    ...
    /**
     * Съобщене при подписа за TRYTIPE 1
     * За всеки TRYTIPE е различен подпипса. 
     * Полетата може да се видят в документацията предоставена от банката
    ...
    /**
     * Префоратиране на полето ORDER
     * Много важно е да се зане че това всъщност не е order_id-то в платформата/магазина а номера на плащането
     * Трябва да е уникален за деня 6 циферен int.
     * Примерно 000001, 000002, 001001 и т.н
     * @param $payment_id
     * @return string
     */
     ...
     * Това е реално може да се разгледа като комбинация от индентификатора за плащане (ORDER) в борика и индентификатора за поръчка в ситемата
     ...
     * @param string $divider стринг с който разделя двете части на AD.CUST_BOR_ORDER_ID
     * @return string
     */
    ...
    function generateBoricaRequestFieldsForTRTYPE_1()
    {
        ...
        $amount = 22;
        //Коректен формат за стойноста на проъчката e {xxx.xx} примерно 22.00
        $amount = number_format($amount, 2, '.', '');
        //Дата в коректен формат
        $timestamp = date('YmdHis');
        /**
         * Валутата за която е настроен терминала от банката
         * !!! ВАЖНО ТАЗИ ВАЛУТА Е САМО ТАЗИ КОЯТО Е НАСТОРЕНА В ТЕРМИНАЛА ОТ БАНКАТА
         */
        $currency = 'BGN';
